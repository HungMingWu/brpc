...
int Socket::Create(const SocketOptions& options, SocketId* id) {
    ...
    m->_recycle_flag.store(false, std::memory_order_relaxed);
    m->_error_code = 0;
    m->_error_text.clear();
...
int Socket::WaitAndReset(int32_t expected_nref) {
    ...
    CHECK(NULL == _write_head.load(std::memory_order_relaxed));
    CHECK_EQ(0, _unwritten_bytes.load(std::memory_order_relaxed));
    CHECK(!_overcrowded);
    return 0;
}
...
void Socket::Revive() {
    ...
    while (1) {
        ...
        if (_versioned_ref.compare_exchange_weak(
            ...
            _recycle_flag.store(false, std::memory_order_relaxed);
            if (_user) {
...
void Socket::DebugSocket(std::ostream& os, SocketId id) {
    ...
    os << "\npipeline_q=" << npipelined
       ...
       << "\nrecycle_flag=" << ptr->_recycle_flag.load(std::memory_order_relaxed)
       << "\nagent_socket_id=";
